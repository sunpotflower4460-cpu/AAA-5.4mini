...
export function AppShell({ children }: AppShellProps) {
  return (
    <div className="relative min-h-[100dvh] overflow-hidden text-[color:var(--color-sumi)]">
      ...
      />
      <div className="relative mx-auto flex min-h-[100dvh] w-full max-w-[860px] flex-col px-4 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-[max(0.85rem,env(safe-area-inset-top))] sm:px-5">
        {children}
      </div>
    </div>
  );
